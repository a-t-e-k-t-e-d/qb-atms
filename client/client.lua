    ...
    if cards ~= nil and cards[1] ~= nil then
        ...
        for k, v in pairs(Config.ATMModels) do
            ...
            if atm then
                ...
                QBCore.Functions.Progressbar("accessing_atm", Lang:t('success.accessing_atm'), 1500, false, true, {
        ...
        QBCore.Functions.Notify(Lang:t("error.buy_card"), "error")
    end
end)
...
RegisterNUICallback("removeCard", function(data, cb)
    QBCore.Functions.TriggerCallback('qb-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            ...
            QBCore.Functions.Notify(Lang:t("success.card_delete"), 'success')
        else
            QBCore.Functions.Notify(Lang:t("error.card_delete"), 'error')
        end
    end, data)
end)
